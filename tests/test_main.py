...
@pytest.mark.parametrize("dataset", [
   pytest.lazy_fixture('coco_dataset'), 
   pytest.lazy_fixture('unlabeled_dataset')
   ])

def test_num_images(dataset):
   assert isinstance(dataset.analyze.num_images, int), "analyze.num_images should return an int"

def test_df_is_dataframe(coco_dataset):
   assert isinstance(coco_dataset.df, pd.DataFrame), "dataset.df should be a pandas dataframe"

def test_num_classes(coco_dataset):
   assert isinstance(coco_dataset.analyze.num_classes, int), "analyze.num_images should return an int"

def test_classes(coco_dataset):
   assert isinstance(coco_dataset.analyze.classes, list), "analyze.classes should return a list"

def test_export_coco(coco_dataset):
   path_to_coco_export = coco_dataset.export.ExportToCoco()
   assert isinstance(path_to_coco_export[0], str), "ExportToCoco should return a list with one or more strings."